...
public class FrontServlet extends HttpServlet {
    ...
    private void process(Route route, HttpServletRequest request, HttpServletResponse response) throws NotFoundException, ServletException, IOException {
        ...
        Method method = findActionMethod(route, pageClass);
        ...
        try {
            ...
            for (Object parameter : parameterTypes) {
                if (HttpServletRequest.class.equals(parameter)) {
                    parameters[parameterCount++] = request;
                } else if (Map.class.equals(parameter)) {
                    parameters[parameterCount++] = view;
                } else {
                    throw new NotFoundException();
                }
            }
        ...
        try {
            ...
            if (sourceConfiguration == null) {
                throw new ServletException("Can't render template [pageClass="
                        + pageClass + ", action=" + method + "]", exception);
            }
        }
    }

    private Method findActionMethod(Route route, Class<?> pageClass) throws NotFoundException {
        Method method = null;
        for (Class<?> clazz = pageClass; method == null && clazz != null; clazz = clazz.getSuperclass()) {
            for (Method m : clazz.getDeclaredMethods()) {
                if (m.getName().equals(route.getAction()) && m.getReturnType() == void.class) {
                    ...
                    for (Object parameterType: m.getParameterTypes()) {
                        if (!HttpServletRequest.class.equals(parameterType) && !Map.class.equals(parameterType)) {
                            ignore = true;
                            break;
                        }
                    }
    ...
    private void setLanguage(HttpServletRequest request) {
        ...
        if (language != null && language.matches("[a-z][a-z]")) {
